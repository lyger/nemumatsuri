...
export function Star({ tilt, scale, animated, burst }) {
  ...
  return (
    <motion.g
      ...
      {burst && <Burst r={150} color={dressingColors.STAR} period={10 * scale - 0.2 * tilt} />}
      <motion.path
        ...
        style={{fill: dressingColors.STAR}} />
    </motion.g>
  );
}
...
export function Moon({ tilt, r }) {
  ...
  const [ { dressingColors } ] = useContext(ColorContext);
  const upperColor = (phase + 0.25) % 1 < 0.5 ? dressingColors.MOONSHADOW : dressingColors.STAR;
  useEffect(() => { setPhase(getMoonPhase()); }, []);
  return (
    <motion.g style={{rotate: tilt}}>
      <circle r={r} cx={0} cy={0} style={{fill: dressingColors.STAR}} />
      <path
        d={`M0,${-r} a1,1 0 0,${phase < 0.5 ? 0 : 1} 0,${2 * r}`}
        style={{fill: dressingColors.MOONSHADOW}} />
      <path
