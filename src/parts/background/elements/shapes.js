...
export function Star({ tilt, scale, animated, burst }) {
  ...
  return (
    <motion.g
      ...
      {burst && <Burst r={150} color={backgroundColors.STAR} period={10 * scale - 0.2 * tilt} />}
      <motion.path
        ...
        style={{fill: backgroundColors.STAR}} />
    </motion.g>
  );
}
...
export function Moon({ tilt, r }) {
  const [phase, setPhase] = useState(0);
  const [ { backgroundColors } ] = useContext(ColorContext);
  const upperColor = (phase + 0.25) % 1 < 0.5 ? backgroundColors.MOONSHADOW : backgroundColors.STAR;
  useEffect(() => { setPhase(getMoonPhase()); }, []);
  return (
    <motion.g style={{rotate: tilt}}>
      <circle r={r} cx={0} cy={0} style={{fill: backgroundColors.STAR}} />
      <path
        d={`M0,${-r} a1,1 0 0,${phase < 0.5 ? 0 : 1} 0,${2 * r}`}
        style={{fill: backgroundColors.MOONSHADOW}} />
      <path
